...
def get_or_create_device(device_sn: str, client_ip: str = "") -> Dict[str, Any]:
    ...
    if device_sn not in DEVICES:
        ...
        log(f"🆕 New device detected: {device_sn} from {client_ip}")
...
async def device_command_endpoint(request: Request):
    ...
    if device_sn in COMMAND_QUEUE and COMMAND_QUEUE[device_sn]:
        command = COMMAND_QUEUE[device_sn].pop(0)
        
        # Store outgoing command as raw data
        store_raw_data(device_sn, command, "OUTGOING", client_ip)
        log(f"📤 Sending to {device_sn}: {command}")
        
        return PlainTextResponse(command)
    
    # Default response - request more data
    default_command = "GET ATTLOG"
    store_raw_data(device_sn, default_command, "OUTGOING", client_ip)
    return PlainTextResponse(default_command)
...
async def fetch_all_devices():
    ...
    log(f"🚀 Initiated data fetch for {devices_count} devices")
    
    return JSONResponse({
        "status": "fetch_initiated",
        "devices_count": devices_count,
        "message": f"Data fetch initiated for {devices_count} devices"
    })
